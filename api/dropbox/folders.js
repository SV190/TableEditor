...
async function getDropboxFolders(accessToken, path = '') {
  ...
  if (!response.ok) throw new Error('Failed to get Dropbox folders');
  const data = await response.json();
  return data.entries.filter(entry => entry[".tag"] === "folder").map(entry => ({
    id: entry.id,
    name: entry.name,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const folders = await getDropboxFolders(accessToken, path);
    res.status(200).json(folders);
  } catch (error) {
    res.status(500).json({ error: 'Ошибка получения папок Dropbox' });
  }
} 